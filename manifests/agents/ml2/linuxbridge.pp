...
class neutron::agents::ml2::linuxbridge (
  ...
  Neutron_plugin_ml2<||> ~> Service['neutron-plugin-linuxbridge-agent']
  ...
  if ('vxlan' in $tunnel_types) {
    ...
    if $vxlan_group {
      neutron_plugin_ml2 { 'vxlan/vxlan_group': value => $vxlan_group }
    } else {
      neutron_plugin_ml2 { 'vxlan/vxlan_group': ensure => absent }
    }

    if $vxlan_ttl {
      neutron_plugin_ml2 { 'vxlan/vxlan_ttl': value => $vxlan_ttl }
    } else {
      neutron_plugin_ml2 { 'vxlan/vxlan_ttl': ensure => absent }
    }

    if $vxlan_tos {
      neutron_plugin_ml2 { 'vxlan/vxlan_tos': value => $vxlan_tos }
    } else {
      neutron_plugin_ml2 { 'vxlan/vxlan_tos': ensure => absent }
    }
  ...
  if $firewall_driver {
    neutron_plugin_ml2 { 'securitygroup/firewall_driver': value => $firewall_driver }
  } else {
